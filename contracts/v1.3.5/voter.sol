 ...
 *
 *  Version: 1.3.5
 *  - Restrict Fee-Claims to once per veNFT per epoch per pool
 *
 *
 *  Contributors:
...
contract Voter is Initializable {
    ...
    function _vote(uint _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
    	...
    	for(uint lol=0;lol<_poolVote.length;lol++) {
    		require(
    			_poolVote[lol] != 0x846267990B6398bef102F6FFa858dAD603048ab3,
    			"voting for sAMM-EQUAL/WFTM pool not allowed :("
    		);
    	}
        _reset(_tokenId);
        uint _poolCnt = _poolVote.length;
    ...
    function claimFees(address[] memory _fees, address[][] memory _tokens, uint _tokenId) external {
        ...
        for (uint i = 0; i < _fees.length; i++) {
        	require(
        		...
        		"Equalizer v1.3.5 : Fees Claims are allowed only once every epoch! Please try again in the next epoch."
        	);
            lastFeesClaimTime[_tokenId][_fees[i]] = _now;
            try IBribe(_fees[i]).getRewardForOwner(_tokenId, _tokens[i]) { }
            catch {require(false, "Equalizer v1.3.5 : Fees Claim expired! Please try again in the next epoch.");}
        }
    }

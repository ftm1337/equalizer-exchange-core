 ...
 *  	- Prevent voting for sAMM-EQUAL/WFTM pool, which mistakenly got bribed $2500 by the team..
 *  		- 0x846267990B6398bef102F6FFa858dAD603048ab3
 *  	- Simultaneously, the Gauge for this pool should be Killed
 *
 *
 *  Contributors:
...
contract Voter is Initializable {
    ...
    mapping(address => address) public bribes; // gauge => all bribes
	...
	/// Events
    event GaugeCreated(address indexed pool, address indexed gauge, address indexed bribe, address creator);
    event GaugeKilled(address indexed gauge);
    event GaugeRevived(address indexed gauge);
    event Voted(address indexed voter, address indexed tokenOwner, uint indexed tokenId, uint256 weight);
    ...
    function _vote(uint _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint i = 0; i < _poolCnt; i++) {
            ...
            if (isGauge[_gauge]) {
                ...
                emit Voted(msg.sender, _tokenOwner, _tokenId, _poolWeight);
            }
        }
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        _updateFor(_gauge);
        pools.push(_pool);
        emit GaugeCreated(_pool, _gauge, _bribe, msg.sender);
        return _gauge;
    }

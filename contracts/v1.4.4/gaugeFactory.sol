...
contract Gauge is Initializable {
    ...
    event ClaimFees(address indexed from, address indexed to, uint claimed0, uint claimed1);
    event ClaimFees(address indexed from, address indexed to, address indexed _token, uint _amount);    /// For Non-Native Gauges
    ...
    function _claimFees() internal returns (uint claimed0, uint claimed1)  {
        /// Equa7izer v1.4: Support Custom pools to be Gaugable
        if (!isForPair) {
            ...
            try IPair(stake).claimFees() {
                ...
                for(uint _n = 0; _n < _bn; _n++) {
                    uint _a =
        	            ...
        	            ? 0
        	            : _brews[_n].balanceOf(address(this));
                    /// Don't trigger bribes for 0x00 rewards, as 0 - 0 !> 0
                    if(_a > _brewbals[_n]) {
                        if (feeTaker == address(0)) {
                            IBribe(bribe).notifyRewardAmount( address(_brews[_n]), (_a - _brewbals[_n]) );
                            emit ClaimFees(msg.sender, bribe, address(_brews[_n]), (_a - _brewbals[_n]) );
                        }
                        else {
                            _brews[_n].transfer(feeTaker, (_a - _brewbals[_n]) );
        ...
        uint t0ba = IERC20(_token0).balanceOf(address(this));
        uint t1ba = IERC20(_token1).balanceOf(address(this));
        claimed0 = t0ba - t0bb;
        claimed1 = t1ba - t1bb;

		if (feeTaker == address(0)) {
        	if (claimed0 > 0) {
            	fees0 += claimed0;	// fee0 stores total token0 fees claimed since genesis
            	IBribe(bribe).notifyRewardAmount(_token0, claimed0);
        	}
        	if (claimed1 > 0) {
            	fees1 += claimed1;	// fee1 stores total token1 fees claimed since genesis
            	IBribe(bribe).notifyRewardAmount(_token1, claimed1);
        	}
            emit ClaimFees(msg.sender, bribe, claimed0, claimed1);
        }

        else {
        	IERC20(_token0).transfer(feeTaker, claimed0);
        	IERC20(_token1).transfer(feeTaker, claimed1);
            emit ClaimFees(msg.sender, feeTaker, claimed0, claimed1);
        }
    ...
    */
    ...
    function getReward(address account, address[] memory tokens) external lock {
        require(msg.sender == account || msg.sender == voter, "Un-authorized claim!");
        _locked = false;
    ...
    function deposit(uint amount, uint tokenId) public lock {
        ...
        if (tokenId > 0) {
            ...
            require(tokenIds[msg.sender] == tokenId, "Please re-use old veNFT attached to this gauge!");
        } else {
    ...
    function withdrawToken(uint amount, uint tokenId) public lock {
        ...
        if (tokenId > 0) {
            require(tokenId == tokenIds[msg.sender], "Please re-use old veNFT attached to this gauge!");
            tokenIds[msg.sender] = 0;
            IVoter(voter).detachTokenFromGauge(tokenId, msg.sender);	/// Signifies total exit
        } else {
            tokenId = tokenIds[msg.sender];
        }
    ...
    function notifyRewardAmount(address token, uint amount) external lock {
        require(token != stake, "Can't distribute staked token as reward!");
        ...
        if (block.timestamp >= periodFinish[token]) {
            ...
            amount = rtba - rtbb;
            require(amount > 0, "Amount must be greater than 0!");
            require(amount > _left, "Griefing Protection enabled!");
            rewardRate[token] = (amount + _left) / DURATION;
        }
        require(rewardRate[token] > 0, "Provided reward too Low!");
        uint balance = IERC20(token).balanceOf(address(this));
        require(rewardRate[token] <= balance / DURATION, "Provided reward too high || Missing Rewards!");
    ...
    function swapOutRewardToken(uint i, address oldToken, address newToken) external {
        ...
        require(rewards[i] == oldToken, "Token index i mismatch!");

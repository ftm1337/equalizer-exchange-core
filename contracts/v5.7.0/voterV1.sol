...
/**v3.0.4
 *0xa8bc72582856b0d4939e30988773df882f849aa2
...
/**
 *v2.0.2
 *0x4556ad146b1e278d05b6fcd758e14a5e52949e32
 *Submitted for verification at FtmScan.com on 2023-06-10
*/
...
contract Voter is Initializable {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////// DO NOT DELETE ANYTHING. DO NOT MOVE ANYTHING. ONLY APPEND TO THE TAIL! ////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////

    mapping(address => bool) public pausedGauges;
    mapping(address => address) public customGauges; // pool => customGauges

    mapping(uint256 => uint256) public voterTurnouts; // epochStart -> votes
    mapping(uint256 => mapping(address => uint256)) public voteResults;  // epochStart -> pool -> votes

    mapping(address => uint256) public lastDistribution; // gauges -> dist.time
    mapping(uint256 => mapping(address => uint256)) public distributions; // epoch -> gauges -> notified
	...
	mapping(address=>bool) public isListingsManager;
	...
	event ListingsManagerSet(address indexed lister, bool indexed status);
    ...
    function initialSetup(address[] memory _tokens, address[] memory _gaugables, address _minter) external {
        require(msg.sender == minter, "Not minter!");
        for (uint i = 0; i < _tokens.length; i++) {
            _whitelist(_tokens[i]);
        }
        for (uint i = 0; i < _gaugables.length; i++) {
            gaugable[_gaugables[i]] = true;
        }
        minter = _minter;
    }
    ...
    function _reset(uint _tokenId) internal {
        ...
        for (uint i = 0; i < _poolVoteCnt; i ++) {
            ...
            if (_votes != 0) {
                ...
                //// Disallow 0-votes on a gauge
                require( weights[_pool] > 0 , "Last remaining voter cant unvote!");
                votes[_tokenId][_pool] -= _votes;
                IBribe(bribes[_gauge])._withdraw(uint256(_votes), _tokenId, msg.sender, _tokenOwner);
                _totalWeight += _votes;
    ...
    function _vote(uint _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint i = 0; i < _poolCnt; i++) {
            ...
            if (isGauge[_gauge]) {
                ...
                voteResults[ (block.timestamp / DURATION) * DURATION ][ _poolVote[i] ] += _poolWeight;
            }
        }
        if (_usedWeight > 0) IVotingEscrow(_ve).voting(_tokenId);
        totalWeight += uint256(_totalWeight);
        usedWeights[_tokenId] = uint256(_usedWeight);
        voterTurnouts[ (block.timestamp / DURATION) * DURATION ] += uint256(_usedWeight);
    }
    ...
    function createGauge(address _pool) public returns (address) {
        ...
        if (
        	(msg.sender != governor)
        	&& (isListingsManager[msg.sender] == false)
        ) { // gov can create for any pool, even non-Equalizer pairs
    ...
    function _ensureOldDist(address _gauge) internal view {
    	uint _ld = lastDistribution[_gauge];
    	if(_ld != 0) { // check only on old
    		uint _bt = block.timestamp;
        	require(_ld > _bt - _bt % DURATION, "Distribute gauge first!");
        }
    }
    ...
    function distribute(address _gauge) public lock {
        ...
        lastDistribution[_gauge] = block.timestamp;
        uint _claimable = claimable[_gauge];
        if (_claimable > IGauge(_gauge).left(base) && _claimable / DURATION > 0) {
        	{
        		uint _now = block.timestamp;
        		distributions[ _now - (_now % 7 days) ][ _gauge ] += _claimable;
        	}
            claimable[_gauge] = 0;
            if(pausedGauges[_gauge] == true) {
            	IERC20(base).transfer(ms, _claimable);
        		emit DistributeReward(msg.sender, ms, _claimable);
            }
            else {
        		if(customGauges[_gauge] != address(0)) {
        			IGauge(customGauges[_gauge]).notifyRewardAmount(base, _claimable);
        			emit DistributeReward(msg.sender, customGauges[_gauge], _claimable);
        		}
        		else {
    ...
    function setListingsManager(address _m, bool _b) external {
        require(msg.sender == governor, "Not governor");
        isListingsManager[_m] = _b;
        emit ListingsManagerSet(_m, _b);
    }

    ///////////////////////////////////////// LISTING MANAGEMENT /////////////////////////////////////////////

    function whitelist(address[] calldata _tokens) external {
        require(msg.sender == governor || isListingsManager[msg.sender], "Not governor");
        for (uint i = 0; i < _tokens.length; i++) {
            _whitelist(_tokens[i]);
        }
    }
    ...
    function setCustomGauge(address[] calldata _g, address[] calldata _cg) external {
        require(msg.sender == governor || isListingsManager[msg.sender], "Not governor");
        for (uint i = 0; i < _g.length; i++) {
            address _og = customGauges[ _g[i] ] ;
            if(_og != address(0)) {
				IERC20(base).approve(_og, 0);
            }
            customGauges[ _g[i] ] = _cg[i] ;
            if( _cg[i] != address(0) ) {
				IERC20(base).approve( _cg[i], type(uint).max);
            }
        }
    }
